...
MRPT_INITIALIZER(do_register_MolaVizImGui)  // NOLINT
{
  ...
  // TODO: register default sensor-observation GUI handlers (see
  //       MolaVizImGui_handlers.cpp once implemented).
}
...
void MolaVizImGui::gui_thread()
{
  ...
  while (!guiThreadShutdown_.load())
  {
    ...
    // Drain task queue (all windows share one queue for simplicity):
    {
      task_queue_t tasks;
      {
        std::lock_guard lk(guiThreadPendingTasksMtx_);
...
{
  // TODO: render wd.background_scene into an FBO, then display as an
  // ImGui::Image() that fills the dockspace central node.
  // This requires:
  //   1. Create/resize FBO to match the central node size each frame.
  //   2. Bind FBO, render mrpt scene via its OpenGL renderer.
  //   3. Unbind FBO.
  //   4. ImGui::Image(reinterpret_cast<ImTextureID>(fbo_texture), node_size).
  // Stub: just clear to background colour (already done in render_frame).

  // TODO: Use the provided helper from mrpt's next version mrpt::imgui
  (void)wd;
}
...
{
  auto task = std::make_shared<std::packaged_task<bool()>>(
      [this, obj, subWindowTitle, extra_parameters, parentWindow]()
      {
        const char*     cls = obj->GetRuntimeClass()->className;
        auto&           hc  = ImGuiHandlersContainer::Instance();
        std::lock_guard lk(hc.mtx);
        bool            any = false;
        for (auto [it, end] = hc.handlers.equal_range(cls); it != end; ++it)
        {
          it->second(obj, nullptr /*no raw ptr*/, parentWindow, this, extra_parameters);
          any = true;
        }
        return any;
      });
