...
{
  for (const auto& anyW : tab.widgets) render_any_widget(anyW);
}
  ...
  std::visit(
      [](auto&& widget)
      {
        ...
        else if constexpr (std::is_same_v<T, mola::gui::TextBox>)
        {
          ...
          if (it == buffers.end()) it = buffers.emplace(widget.label, widget.initial_value).first;
        ...
        else if constexpr (std::is_same_v<T, mola::gui::TextPanel>)
        {
          ...
          if (widget.editable)
          {
            ...
            static std::map<const mola::gui::LiveString*, std::string> bufs;
            auto*                                                      key = widget.live_text.get();
            auto                                                       it  = bufs.find(key);
            if (it == bufs.end())
            {
              // Seed from LiveString on first use only:
              widget.live_text->pollIntoDisplay();
              it = bufs.emplace(key, widget.live_text->display).first;
            }
            const std::string id = "##tpe_" + std::to_string(reinterpret_cast<uintptr_t>(key));
          ...
          else
          {
            ...
            widget.live_text->pollIntoDisplay();
            const std::string id =
                "##tpr_" + std::to_string(reinterpret_cast<uintptr_t>(widget.live_text.get()));
